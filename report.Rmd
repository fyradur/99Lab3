...
# Statement of Contribution
 ...
 - John Möller (johmo870) - Assignment2 (did not complete in time), Report
...
(Not complete, bugs still present)
...
gaussian_physical <- function(x1, x2) {
  dist <- distHaversine(as.numeric(x1[c(5,4)]), as.numeric(x2[c(5,4)])) # Distance in meters
...
## Prediction
```{r}
predict_temperature_iteratively <- function(st, target_lat, target_lon, target_date, target_times, kernel_function) {
  M <- length(target_times)  # Number of target time points
  temp_predictions <- numeric(M)  # Initialize vector for predictions
  
  # Iterate over each target time
  for (j in 1:M) {
    target_time <- target_times[j]
    kernel_values <- numeric(nrow(st))  # Initialize kernel values for all measurements
    
    # Iterate over each station measurement
    for (i in 1:nrow(st)) {
      x1 <- c(target_lat, target_lon, target_date, target_time)  # Target point
      x2 <- st[i, ]  # Station data row
      
      # Compute the kernel value using the provided function
      kernel_values[i] <- kernel_function(x1, x2)
    }
    
    # Calculate the weighted sum of temperatures
    numerator <- sum(kernel_values * st$temperature)
    denominator <- sum(kernel_values)
    
    # Store the predicted temperature
    temp_predictions[j] <- numerator / denominator
  }
  
  return(temp_predictions)
}

prediction_sum <- predict_temperature_iteratively(st, a, b, date_to_predict, times, gaussian_sum)
prediction_prod <- predict_temperature_iteratively(st, a, b, date_to_predict, times, gaussian_prod)
```

## Plots

```{r}
plot(prediction_sum, type = "o")
plot(prediction_prod, type = "o")
```
